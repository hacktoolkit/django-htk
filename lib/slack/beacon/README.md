...
```python
from htk.lib.lib.slack.beacon.utils import Client

# Create client with credentials
client = Client(api_key='your_api_key')

# Or use from settings
client = Client()  # Uses HTK_BEACON_API_KEY from settings
```
...
```python
# settings.py
HTK_BEACON_API_KEY = 'your_api_key'
HTK_BEACON_API_SECRET = 'your_secret'
HTK_BEACON_API_URL = 'https://api.service.com'
```
...
```python
# settings.py
HTK_BEACON_ENABLED = True
HTK_BEACON_API_KEY = 'your_key'
HTK_BEACON_TIMEOUT = 30
HTK_BEACON_RETRIES = 3
```
