...
class HtkShopifyArchiver(object):
    ...
    def archive_item_type(self, item_type):
        ...
        msg = 'Archived %s %ss in %s seconds' % (
            num_archived,
            item_type,
            duration,
        )
...
class HtkShopifyMongoDBArchiver(HtkShopifyArchiver):
    ...
    def upsert(self, item_type, document):
        ...
        if self.already_cached(item_type, document, key):
            if item_type not in self.fk_item_types:
                print(
                    'Skipping duplicate processed in session {}: {}'.format(
                        item_type, pk
                    )
                )
                print(document)
    ...
    def archive_product(self, item_type, product):
        ...
        image_id = self._archive_product_image(
            'product_image', product_image, pk
        )
        document['image_id'] = image_id
        del document['image']
        # images -> image_ids (fk)
        image_ids = [
            self._archive_product_image('product_image', product_image, pk)
            for product_image in document.get('images', [])
        ]
        ...
        variant_ids = [
            self._archive_product_variant(
                'product_variant', product_variant, pk
            )
            for product_variant in document.get('variants', [])
        ]
    ...
    def archive_customer(self, item_type, customer):
        ...
        default_address_id = (
            self._archive_customer_address('customer_address', default_address)
            if default_address
            else None
        )
        ...
        address_ids = [
            self._archive_customer_address('customer_address', customer_address)
            for customer_address in document.get('addresses', [])
        ]
    ...
    def archive_order(self, item_type, order):
        ...
        line_item_ids = [
            self._archive_order_line_item(
                'order_line_item', order_line_item, pk
            )
            for order_line_item in document.get('line_items', [])
        ]
        ...
        fulfillment_ids = [
            self._archive_fulfillment('fulfillment', fulfillment, pk)
            for fulfillment in document.get('fulfillments', [])
        ]
        ...
        refund_ids = [
            self._archive_refund('refund', refund, pk)
            for refund in document.get('refunds', [])
        ]
    ...
    def _archive_refund(self, item_type, document, order_id):
        ...
        transaction_ids = [
            self._archive_transaction(
                'transaction', transaction, order_id, refund_id=pk
            )
    ...
    def _archive_transaction(
        self, item_type, document, order_id, refund_id=None
    ):
        pk = document['id']
        del document['id']
        document['_id'] = pk
...
class HtkShopifySQLArchiver(HtkShopifyMongoDBArchiver):
    ...
    def _db_upsert(self, item_type, document, pk):
        ...
        if self.mongodb_dual_archive:
            try:
                super(HtkShopifySQLArchiver, self)._db_upsert(
                    item_type, document, pk
                )
            except:
                rollbar.report_exc_info()
