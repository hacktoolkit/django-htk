...
```python
from htk.lib.lib.google.sheets.utils import Client

# Create client with credentials
client = Client(api_key='your_api_key')

# Or use from settings
client = Client()  # Uses HTK_SHEETS_API_KEY from settings
```
...
```python
# settings.py
HTK_SHEETS_API_KEY = 'your_api_key'
HTK_SHEETS_API_SECRET = 'your_secret'
HTK_SHEETS_API_URL = 'https://api.service.com'
```
...
```python
# settings.py
HTK_SHEETS_ENABLED = True
HTK_SHEETS_API_KEY = 'your_key'
HTK_SHEETS_TIMEOUT = 30
HTK_SHEETS_RETRIES = 3
```
