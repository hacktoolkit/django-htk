...
```python
from htk.lib.lib.google.recaptcha.utils import Client

# Create client with credentials
client = Client(api_key='your_api_key')

# Or use from settings
client = Client()  # Uses HTK_RECAPTCHA_API_KEY from settings
```
...
```python
# settings.py
HTK_RECAPTCHA_API_KEY = 'your_api_key'
HTK_RECAPTCHA_API_SECRET = 'your_secret'
HTK_RECAPTCHA_API_URL = 'https://api.service.com'
```
...
```python
# settings.py
HTK_RECAPTCHA_ENABLED = True
HTK_RECAPTCHA_API_KEY = 'your_key'
HTK_RECAPTCHA_TIMEOUT = 30
HTK_RECAPTCHA_RETRIES = 3
```
