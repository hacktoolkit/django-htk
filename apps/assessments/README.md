...
> Assessment and survey management system.

## Purpose

The assessments app provides creation, distribution, and evaluation of surveys and assessments.
...
| Component | Purpose |
|-----------|---------|
| **Models** | Assessment, Question, Response models |
...
|----------|--------|---------|
| `/api/assessments/` | GET | List items |
| `/api/assessments/` | POST | Create item |
| `/api/assessments/{id}/` | GET | Get item details |
| `/api/assessments/{id}/` | PATCH | Update item |
| `/api/assessments/{id}/` | DELETE | Delete item |

## Configuration

```python
# settings.py
HTK_ASSESSMENTS_ENABLED = True
...
class AssessmentsTestCase(TestCase):
    def setUp(self):
