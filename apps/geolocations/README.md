...
The geolocations app provides IP geolocation, coordinate tracking, distance queries, and location analytics.

## Quick Start

```python
from htk.apps.geolocations.models import *
...
| Component | Purpose |
|-----------|---------|
| **Models** | Geolocation, CoordinateTrace models |
...
|----------|--------|---------|
| `/api/geolocations/` | GET | List items |
| `/api/geolocations/` | POST | Create item |
| `/api/geolocations/{id}/` | GET | Get item details |
| `/api/geolocations/{id}/` | PATCH | Update item |
| `/api/geolocations/{id}/` | DELETE | Delete item |

## Configuration

```python
# settings.py
HTK_GEOLOCATIONS_ENABLED = True
